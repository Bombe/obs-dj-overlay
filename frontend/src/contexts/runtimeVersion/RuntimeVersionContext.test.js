...
describe('The Runtime Version Context', () => {
    ...
    it('should provide the version from the runtime service context', async () => {
        ...
        await act(async () => {
            render(<WithRuntimeService runtimeService={runtimeService}><WithRuntimeVersion><RuntimeVersionContext.Consumer>{storeProvidedValue}</RuntimeVersionContext.Consumer></WithRuntimeVersion></WithRuntimeService>)
        })

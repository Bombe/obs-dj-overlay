...
describe("The Export Collector", () => {

    it("should collect nothing for empty history", () => {
        expect(collector.collect(historyEntries)).to.eql([])
    })

    it("should collect nothing if range in invalid", () => {
        expect(collector.collect(historyEntries, undefined, undefined)).to.eql([])
    })

    it("should collect nothing if to is before from", () => {
        expect(collector.collect(historyEntries, "20200908T1100", "20200908T1000")).to.eql([])
    })

    it("should include the first track if before is earlier than the first track", () => {
        expect(collector.collect(historyEntries, "20200908T0900", "20200908T1200")).to.eql(historyEntries)
    })
